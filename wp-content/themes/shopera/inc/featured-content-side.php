 ...
 */
class Shopera_Featured_Content_Side {
	 ...
	 * passed in the call to add_theme_support( 'featured-content-side' ).
	 *
	 * @see Featured_Content_Side::init()
	 *
	 * @since Shopera 1.0
	 *
	 * @static
	 ...
	 * add_theme_support( 'featured-content-side' ); during after_setup_theme.
	 *
	...
	public static function init() {
		$theme_support = get_theme_support( 'featured-content-side' );
		...
		// Return early if "featured_content_side_filter" has not been defined.
	 ...
	 *
	 * Sets the "featured_content_side_ids" transient.
	 *
	 * @static
	 * @access public
	 * @since Shopera 1.0
	...
	public static function get_featured_post_ids() {
		...
		$featured_ids = get_transient( 'featured_content_side_ids' );
		...
		set_transient( 'featured_content_side_ids', $featured_ids );
	 ...
	 *
	 * @see Featured_Content_Side::validate_settings().
	 *
	 * @static
	 * @access public
	 * @since Shopera 1.0
	...
	public static function delete_transient() {
		delete_transient( 'featured_content_side_ids' );
	}
	 ...
	 * the transient is deleted by Featured_Content_side::validate_settings().
	 *
	...
	public static function delete_post_tag( $tag_id ) {
		...
		update_option( 'featured-content-side', $settings );
	}
	 ...
	 *
	 * @uses Featured_Content_Side::get_setting()
	 */
	...
	public static function register_setting() {
		register_setting( 'featured-content-side', 'featured-content-side', array( __CLASS__, 'validate_settings' ) );
	}
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content_side', array(
			...
			'description'    => sprintf( __( 'Use the <a href="%1$s">"featured" tag</a> to feature your posts. You can change this to a tag of your choice; if no posts match the tag, <a href="%2$s">sticky posts</a> will be displayed instead.', 'shopera' ), admin_url( '/edit.php?tag=featured' ), admin_url( '/edit.php?show_sticky=1' ) ),
			'priority'       => 140,
			'theme_supports' => 'featured-content-side',
		) );

		// Add Featured Content settings.
		$wp_customize->add_setting( 'featured-content-side[tag-name]', array(
			'default'              => 'featured',
			'type'                 => 'option',
			'sanitize_js_callback' => array( __CLASS__, 'delete_transient' ),
		) );
		$wp_customize->add_setting( 'featured-content-side[hide-tag]', array(
			'default'              => true,
			'type'                 => 'option',
			'sanitize_js_callback' => array( __CLASS__, 'delete_transient' ),
		) );
		...
		$wp_customize->add_control( 'featured-content-side[tag-name]', array(
			'label'    => __( 'Tag Name', 'shopera' ),
			'section'  => 'featured_content_side',
			'priority' => 20,
		) );

		// $wp_customize->add_control( 'featured-content[hide-tag]', array(
		// 	'label'    => __( 'Don&rsquo;t display tag on front end.', 'shopera' ),
		// 	'section'  => 'featured_content_side',
		// 	'type'     => 'checkbox',
		// 	'priority' => 30,
		// ) );
	...
	public static function get_setting( $key = 'all' ) {
		$saved = (array) get_option( 'featured-content-side' );

		$defaults = array(
			'hide-tag' => 1,
			'quantity' => 6,
			'tag-id'   => 0,
			'tag-name' => 'featured-side',
		);
	 ...
	 * delete the transient set in Featured_Content_side::get_featured_content_side().
	 *
	 * @static
	 * @access public
	 * @since Shopera 1.0
...
} // Featured_Content_side

Shopera_Featured_Content_Side::setup();
